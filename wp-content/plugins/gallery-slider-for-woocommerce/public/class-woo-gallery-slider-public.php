...
class Woo_Gallery_Slider_Public {
	...
	public function __construct( $plugin_name, $version ) {
		...
		add_filter( 'body_class', array( $this, 'wcgs_main_class' ), 100 );
	 ...
	 * @return string
	 */
	public function wcgs_main_class( $classes ) {
		if ( is_product() ) {
	...
	public function remove_gallery_and_product_images() {
		global $product;
		if ( is_product() && ( has_post_thumbnail( $product->get_id() ) || count( $product->get_gallery_image_ids() ) > 0 ) ) {
